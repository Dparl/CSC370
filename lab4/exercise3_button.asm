...
;I/O -> A-to-D Converter -> shows them and what their address is (it's in SRAM)
.equ ADCSRA_BTN=0x7A ;Control and Status Register A
.equ ADCSRB_BTN=0x7B ;Control and Status Register B
.equ ADMUX_BTN=0x7C  ;Multiplexer Selection Register: selects a channel, 
                     ;as ADC can handle up to 8 channels
.equ ADCL_BTN=0x78   ;low of the 10-bit ADC Data register
.equ ADCH_BTN=0x79   ;high of the 10-bit ADC Data register (last 2 bits only)
...
check_button:
	...
	ori r16, 0x40 ; 0x40 = 0b01000000     in single conversion mode turn on ADC start conversion bit to start
	sts	ADCSRA_BTN, r16

	; wait for it to complete, check for bit 6, the ADSC bit
wait:	lds r16, ADCSRA_BTN
		andi r16, 0x40;in single conversion mode the start bit is 0 if conversion is in progress, 1 if complete
		brne wait
